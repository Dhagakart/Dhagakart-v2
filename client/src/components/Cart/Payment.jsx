...
import COD from './cod.png'; // Keep only the COD icon
...
const Payment = () => {
    ...
    const [payDisable, setPayDisable] = useState(false);
    const [view, setView] = useState('payment'); // 'payment' or 'sample'

    const { shippingInfo, cartItems } = useSelector((state) => state.cart);
    const { user } = useSelector((state) => state.user);
    const { error: orderError } = useSelector((state) => state.newOrder);

    // This function will handle the Cash on Delivery order placement
    const handleCodOrder = async () => {
        ...
        const { finalTotal, subtotal, totalGst, shippingCharges } = calculateCartTotals();
        ...
        const order = {
            ...
            paymentInfo: {
                id: `COD-${Date.now()}`,
                status: "Processing", // Status for COD is initially processing
            },
            paidAt: Date.now(), // For COD, you might want to handle this differently on the backend
            itemsPrice: subtotal,
            taxPrice: totalGst,
            shippingPrice: shippingCharges,
            totalPrice: finalTotal,
    ...
    // Placeholder function for handling sample requests
    const handleRequestSample = () => {
        // Here you would integrate with Razorpay or another payment gateway
        enqueueSnackbar("Connecting to payment gateway for sample request...", { variant: "info" });
        // Example: navigate('/sample-payment');
    };
    ...
    const PriceRow = ({ label, value, isTotal = false }) => (
        <div className={`flex justify-between ${isTotal ? 'pt-3 mt-3 border-t border-gray-200' : ''}`}>
            ...
            <span className={`${isTotal ? 'text-lg font-semibold' : 'font-medium text-base'}`}>
                {value}
            </span>
        </div>
    );
    ...
    const renderSampleRequestForm = () => (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-semibold mb-4 sm:mb-6 text-gray-800">Request a Sample</h2>
            <p className="text-sm text-gray-600 mb-6">
                Interested in our products? Request a sample to see the quality for yourself. A nominal fee will be charged for sample delivery, which you can pay on the next step.
            </p>
            
            <div className="space-y-4">
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Full Name</label>
                    <input type="text" value={shippingInfo.name} disabled className="w-full p-2 bg-gray-100 border border-gray-300 rounded-md text-sm cursor-not-allowed" />
                </div>
                 <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Shipping Address</label>
                    <textarea value={`${shippingInfo.address}, ${shippingInfo.city}, ${shippingInfo.state} - ${shippingInfo.pinCode}`} disabled className="w-full p-2 bg-gray-100 border border-gray-300 rounded-md text-sm cursor-not-allowed" rows="3" />
                </div>
                 <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Phone Number</label>
                    <input type="text" value={shippingInfo.phoneNo} disabled className="w-full p-2 bg-gray-100 border border-gray-300 rounded-md text-sm cursor-not-allowed" />
                </div>
            </div>

            <button
                onClick={handleRequestSample}
                className="w-full mt-6 bg-blue-600 text-white py-3 px-4 rounded-md font-medium transition-colors text-base h-12 flex items-center justify-center hover:bg-blue-700 active:bg-blue-800 hover:cursor-pointer"
            >
                Proceed to Request Sample
            </button>
        </div>
    );
                        ...
                        <div className="w-full lg:w-2/3">
                            ...
                            <div className="flex border-b border-gray-200 mb-4">
                                <button
                                    ...
                                    className={`py-3 px-6 font-medium text-center hover:cursor-pointer ${view === 'payment' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    Place Order
                                </button>
                                <button
                                    onClick={() => setView('sample')}
                                    className={`py-3 px-6 font-medium text-center hover:cursor-pointer ${view === 'sample' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    Request Sample
                                </button>
                            </div>
