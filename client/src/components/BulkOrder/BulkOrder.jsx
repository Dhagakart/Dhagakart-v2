...
const BulkOrder = () => {
  ...
  return (
    <div className="min-h-[90vh] max-w-7xl mx-auto mt-4 md:mt-10 py-4 md:py-8 px-4 md:px-16">
      <div>
        <div className="py-4 md:py-6 px-2 md:px-8">
          <h1 className="text-2xl md:text-3xl font-semibold text-gray-900 text-center">Get a Quote instantly</h1>
          <p className="text-gray-600 mt-1 text-center text-sm md:text-base">Share your BBQ/Product Requirement to Dhagakart</p>
        </div>

        <form onSubmit={handleSubmit} onKeyDown={handleFormKeyDown} className="border border-gray-200 rounded-xl pb-6 md:pb-8 px-2 md:px-8">
          {/* Product Details */}
          <div className="pt-4 md:pt-6">
            <h2 className="text-lg md:text-xl font-medium text-gray-800 mb-3 md:mb-4 border-b border-gray-200 pb-2">
              Product Details
            </h2>
            <div className="flex flex-col md:flex-row items-stretch">
              ...
              <div className="flex-1 md:pr-8 mb-4 md:mb-0">
                {!selectedFile ? (
                  <label className="block w-full h-48 md:h-80 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 flex items-center justify-center">
                    <input
                      id="file-upload"
                      type="file"
                      className="hidden"
                    ...
                    <div className="text-center p-2 md:p-4">
                      <div className="mx-auto w-10 h-10 md:w-12 md:h-12 bg-blue-50 rounded-full flex items-center justify-center">
                        <FiUpload className="text-blue-600 w-5 h-5 md:w-6 md:h-6" />
                      </div>
                      <p className="mt-2 text-gray-700 font-medium text-sm md:text-base">Upload Document</p>
                      <p className="text-xs text-gray-500 mt-1">
                        Drag and drop files here or click to browse
                      </p>
                    </div>
                  ...
                  <div className="w-full border-2 border-gray-200 rounded-lg p-2 md:p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 bg-blue-50 p-1 md:p-2 rounded-lg">
                          {filePreview ? (
                            <img
                              src={filePreview}
                              alt="Preview"
                            ...
                            <div className="h-12 w-12 md:h-16 md:w-16 bg-blue-100 rounded flex items-center justify-center">
                              <FiFile className="text-blue-500 w-5 h-5 md:w-6 md:h-6" />
                            </div>
                          )}
                        </div>
                      ...
                      <button
                        type="button"
                        onClick={removeFile}
                        className="text-gray-400 hover:text-gray-500 hover:cursor-pointer ml-2"
                      >
              ...
              {/* Divider with OR */}
              <div className="relative flex md:flex-col items-center justify-center px-0 md:px-4 mb-4 md:mb-0">
                {/* Mobile: horizontal divider */}
                <div className="block md:hidden w-full h-px bg-gray-300 my-2"></div>
                <div className="relative z-10 bg-white px-3 py-1 text-xs md:text-sm text-gray-500 border border-gray-300 rounded-full mx-auto">
                  OR
                </div>
                {/* Desktop: vertical divider */}
                <div className="hidden md:block absolute inset-y-0 w-px bg-gray-300 left-1/2 -translate-x-1/2"></div>
              </div>

              {/* Products & Comments */}
              <div className="flex-1 md:pl-8">
                <div>
                  <h3 className="text-gray-800 font-medium mb-2 md:mb-3 text-base md:text-lg">Add Products</h3>
                  {products.map((item, idx) => (
                    <div key={idx} className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 mb-2 md:mb-3">
                      <div className="relative flex-1">
                        <div className="relative">
                          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <FiSearch className="h-4 w-4 text-gray-400" />
                          ...
                          <input
                            type="text"
                            placeholder="Search Product or Category"
                            className="product-search-input block w-full pl-10 pr-10 py-2 border border-gray-300 rounded-md text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            value={item.product}
                            onChange={e => handleProductChange(idx, 'product', e.target.value)}
                            onKeyDown={e => e.key === 'Enter' && e.preventDefault()}
                            onFocus={() => setShowDropdown(prev => ({ ...prev, [idx]: true }))}
                          ...
                          <div className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto sm:text-sm">
                            {loading[idx] ? (
                              ...
                              searchResults[idx].map(prod => (
                                <div
                                  key={prod._id}
                                  className="cursor-pointer hover:bg-gray-100 px-4 py-2 text-xs md:text-sm text-gray-700"
                                  onClick={() => selectProduct(idx, prod)}
                                >
                                  <div className="font-medium">{prod.name}</div>
                                  <div className="text-xs text-gray-500">
                      ...
                      <input
                        type="text"
                        placeholder="Qty"
                        className="w-full sm:w-20 border border-gray-300 rounded-md px-3 py-2 text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 mt-2 sm:mt-0"
                        value={item.qty}
                        onChange={e => handleProductChange(idx, 'qty', e.target.value)}
                        onKeyDown={e => e.key === 'Enter' && e.preventDefault()}
                      />
                  ...
                  <button
                    type="button"
                    onClick={handleAddProduct}
                    className="inline-flex items-center text-blue-600 text-xs md:text-sm font-medium hover:text-blue-800 mt-1 md:mt-2 hover:cursor-pointer"
                  >
                    <span className="text-lg mr-1">+</span> Add Another Product
                  </button>
                </div>
                <div className="mt-4 md:mt-6">
                  <h3 className="text-gray-800 font-medium mb-1 md:mb-2 text-base md:text-lg">Add Comments</h3>
                  <textarea
                    placeholder="(Optional)"
                    className="w-full border border-gray-300 rounded-md px-3 py-2 text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[80px] md:min-h-[100px] resize-none"
                    value={comments}
                    onChange={e => setComments(e.target.value)}
                  />
                </div>
          ...
          <div className="mt-3 pt-3 w-full flex justify-center items-center border-t border-gray-200">
            <button
              type="submit"
              className="w-full md:w-1/3 bg-[#003366] hover:cursor-pointer text-white py-3 px-6 rounded-md font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors text-base md:text-lg"
            >
              Submit
            </button>
          </div>
